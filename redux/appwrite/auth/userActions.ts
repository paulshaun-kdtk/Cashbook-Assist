...
export const confirmUserAndKey = async (
  ...
  try {
    const queries = [
      ...
      queries.forEach((q, i) => {
        params.append(`queries[${i}]`, JSON.stringify(q));
      });

    const url = `${appwriteCreds.apiUrl}/databases/${appwriteCreds.databaseId}/collections/${appwriteCreds.user_collection_id}/documents?${params}`;
...
export const confirmUserNameBelongsToUser = async (
  ...
  try {
    ...
    const url = `${appwriteCreds.apiUrl}/databases/${appwriteCreds.databaseId}/collections/${appwriteCreds.user_collection_id}/documents?${params.toString()}`;
