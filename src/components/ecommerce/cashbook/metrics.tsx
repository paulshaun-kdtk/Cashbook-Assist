...
import {  selectIncomeStatsCashbook } from "@/redux/api/selectors/incomeTotals";
import {  selectExpenditureStatsCashbook } from "@/redux/api/selectors/expenseTotals";

export const EcommerceMetricsCashbook = ({showBalance=false}) => {
  ...
  const {
    ...
    incomeGrowth,
  } = useSelector(selectIncomeStatsCashbook);
  ...
  const {
    ...
    expenditureGrowth,
  } = useSelector(selectExpenditureStatsCashbook);
  ...
  const balanceGrowthPercentage = ((currentBalance - previousBalance) / Math.abs(previousBalance)) * 100;
      ...
      React.useEffect(() => {
          if (uniqueId) {
            dispatch(fetchIncomeThunkCashbook(uniqueId));
            dispatch(fetchExpensesThunkCashbook(uniqueId));
          }
        }, [dispatch, uniqueId]);
