...
export default function DuplicatesWarnBanner() {
  ...
  const { income_sources} = useSelector((state: any) => state.income_sources);
    ...
    <>
      {!loading && duplicates.length > 0 && (
        <Alert
          ...
          linkHref="/sys/anormalies/duplicates"
        />
      )}
      ...
      {(uniqueId && !showTeamInviteBanner && !accounts.length) && (
        <Alert
          ...
          linkHref="/financial-accounts?new=true"
        />
      )}

      {(uniqueId && !showTeamInviteBanner && !income_sources.length && accounts.length) && (
        <Alert
          variant="info"
          title="Create your first branch"
          message="It seems you haven't added any branches. Please add a branch to continue."
          showLink={true}
          linkText="Add branch"
          linkHref="/income-sources?branches&new=true"
        />
      )}
    </>
  );
