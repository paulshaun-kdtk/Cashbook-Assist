...
interface StockItemsTableProps {
  search: string;
  onFilteredChange?: (filteredItems: Income[]) => void;
}

export default function IncomeTable({ search, onFilteredChange }: StockItemsTableProps) {
    const dispatch = useDispatch()
    const {income, error, loading} = useSelector((state) => state.income)
    const [uniqueId, setUniqueId] = React.useState<string | null>(null);
    const [currentPage, setCurrentPage] = React.useState(1);
    const [selectedItem, setSelectemItem] = React.useState<IncomeExpenseExpense | null>(null);
    ...
    useEffect(() => {
      ...
      const filteredItems = useMemo(() => {
        ...
        return income.filter((item: Income) => {
          ...
          const matchesCategory = searchParams.get("category") ? item?.category === searchParams.get("category") : true;
    ...
    const handleReverseItem = (item: Income) => {
        closeModal();
        dispatch(updateIncomeItemThunk({documentId:item.$id, data:{reversed: !item.reversed}}));
    }

      const handleIncomeDelete = async (income: Income) => {
        const loadingToast = toast.loading("Deleting entry...");
        try {
          const success = await dispatch(deleteIncomeEntry({documentId: income.$id})).unwrap()
          if (!success) {
          ...
          toast.success("Income entry deleted successfully.", {duration: 5000});
          await dispatch(fetchIncomeThunk(uniqueId));
        } catch (error) {
    ...
    const paginatedData:Income[] = filteredItems.slice(startIndex, endIndex)
        ...
        {error && (
            <div className="flex justify-center mt-5">
                    <Badge
                      ...
                      color={"error"}
                      > 
                      <span>Eror fetching income entries - {error}</span>
                     </Badge> 
            </div>
        )}
            ...
            {paginatedData.map((income) => (
                <TableRow key={income.$id}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    ...
                    > {income.is_recurring ? "Yes" : "No"} </Badge>
                  </TableCell>
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    <Badge
                      size="sm"
                    ...
                    > {income.reversed ? "Yes" : "No"} </Badge>
                  </TableCell>
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <Button variant="link_dark" startIcon={<EyeIcon size={18} />} onClick={() => openModal(income)} />

                    {(income.createdAt && new Date().getTime() - new Date(income.createdAt).getTime() < 30 * 24 * 60 * 60 * 1000 && !income.reversed) && (
                      <Button variant="link_warning" startIcon={<FaRotateLeft size={18} />} onClick={() => window.confirm("confirm entry reversal") && handleReverseItem(income)} />
                    )}
                    
                    {(income.createdAt && new Date().getTime() - new Date(income.createdAt).getTime() < 30 * 24 * 60 * 60 * 1000 && income.reversed) && (
                      <>
                        <Button variant="link" startIcon={<FaRotateRight size={18} />} onClick={() => window.confirm("confirm undo entry reversal") && handleReverseItem(income)} />
