...
import { deleteAccountEntry } from "@/redux/api/thunks/accounts/post";
    ...
    useEffect(() => {
        if (uniqueId) {
          ...
          const handleDeleteAccount = async (account: Account) => {
              ...
              try {
                const success = await dispatch(deleteAccountEntry({documentId: account.$id})).unwrap()
                if (!success) {
                ...
                await dispatch(fetchAccountsThunk(uniqueId));
              } catch (error) {
                    ...
                    <div className="flex items-center gap-3">
                      <div>
                        <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                          {formatDateWords(account?.$createdAt)}
                        </span>
                      </div>
                    </div>
                  </TableCell>
