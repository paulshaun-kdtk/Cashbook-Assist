...
import { createCategoryThunk } from '@/redux/thunks/categories/post';
...
export default function AddCategoryForm({onFormSubmit=null}) {
  ...
  const onSubmit = async data => {
    ...
    try {
        const success = await dispatch(createCategoryThunk({data: companyData})).unwrap()
        if (success) {
            toast.showToast({ type: 'success', text1: 'Category added successfully!' });
        }
    } catch (error) {
        console.log(error)
        toast.showToast({ type: 'error', text1: error?.message || 'Category creation failed, please try again.' });
    }
  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-[#0B0D2A] pt-12 min-h-screen">
      <View className="px-4 flex-row items-center justify-center relative mb-6">
        <ThemedText className="text-xl font-bold">Add New Category</ThemedText>
      </View>

      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        <View className="mx-4 rounded-xl overflow-hidden shadow-lg p-4">
          {/* Company Name Input */}
          <ThemedText className="text-sm text-gray-500 dark:text-gray-400 mb-2">Category Name</ThemedText>
          <Controller
            control={control}
            rules={{
              required: true,
          ...
          <ThemedText className="text-sm text-gray-500 dark:text-gray-400 mb-2">Desc</ThemedText>
          <Controller
            control={control}
            rules={{
              required: false,
